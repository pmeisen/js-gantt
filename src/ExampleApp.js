...
require(['jquery', 'net/meisen/general/date/DateLibrary', 'net/meisen/ui/gantt/GanttChart'], function ($, datelib) {
    ...
    var rndSize = 900;
    chartFixedData1.ganttChart().resize(rndSize, 300);
    setInterval(function () {
        rndSize = Math.max(700, Math.random() * 1200);
    }, 5000);
    ...
    var chartLoadedData = $("#chartLoadedData").ganttChart({
        data: {
            url: 'http://localhost:19999/data?type=file&file=resources/sample.csv&separator=;&limit=10000',
            ...
            timeaxis: {
                ...
                end: datelib.createUTC(2014, 12, 31, 23, 59, 0),
                granularity: 'mi'
            }
        },
        illustrator: {
    ...
    var chartFailData = $("#chartFailData").ganttChart({
        data: {
            url: 'http://localhost:19999/data?type=fail',
        }
    });
    ...
    var chartEmptyData1 = $("#chartEmptyData1").ganttChart({
        data: {
            url: 'http://localhost:19999/data?type=file&file=resources/sample.csv&separator=;&limit=0',
            postProcessor: function (data) {
                return {names: data.names, records: data.data};
            }
        }
